...
export default function ProjectReportPage() {
  ...
  const [extraCharges, setExtraCharges] = useState<Array<{ description: string; amount: number }>>([]);
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      const response = await authService.authenticatedFetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080/api/v1'}/projects/${projectId}/report`,
        {
          method: 'POST',
          body: JSON.stringify({
            taskIds: selectedTasks,
            notes: reportNotes || undefined,
            extraCharges: extraCharges.filter(charge => charge.description.trim() && charge.amount > 0),
          }),
        }
      );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push("/employee/projects")} className="mt-4">
            Back to Projects
          </Button>
        </div>
      </div>
  ...
  const selectedTasksDetails = projectTasks.filter(task => selectedTasks.includes(task.taskId));
  const servicesCost = selectedTasksDetails.reduce((sum, task) => sum + (task.estimatedCost || 0), 0);
  ...
  const totalHours = selectedTasksDetails.reduce((sum, task) => sum + (task.estimatedHours || 0), 0);
  ...
  return (
    <div className="min-h-screen p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-primary">Submit Project Report</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{project.customerName || `Customer #${project.customerId || 'N/A'}`}</p>
              </div>
              <div>
              ...
              {project.description && (
                <div className="col-span-2">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 max-h-[400px] overflow-y-auto">
                    {(() => {
                      const parts = project.description.split('--- Project Report ---');
                        ...
                        <div className="space-y-4">
                          {initialText && (
                            <div className="pb-4 border-b border-gray-200">
                              <p className="text-gray-700 text-sm leading-relaxed">{initialText}</p>
                            </div>
                          )}
                          
                          {reportSection && (
                            <div>
                              <h3 className="font-bold text-primary mb-4 text-lg">Project Report</h3>
                              <div className="space-y-3">
                                {reportSection.split('\n').filter(line => line.trim()).map((line, lineIndex) => {
                                  const trimmedLine = line.trim();
                                  
                                  if (trimmedLine.startsWith('Submitted by:')) {
                                    const value = trimmedLine.replace('Submitted by:', '').trim();
                                    return (
                                      <div key={lineIndex} className="flex items-start gap-3">
                                        <span className="text-gray-500 font-medium min-w-[120px]">Submitted by:</span>
                                  ...
                                  if (trimmedLine.startsWith('Date:')) {
                                    const value = trimmedLine.replace('Date:', '').trim();
                                    return (
                                      <div key={lineIndex} className="flex items-start gap-3">
                                        <span className="text-gray-500 font-medium min-w-[120px]">Date:</span>
                                        <span className="text-gray-900">{value}</span>
                                      </div>
                                    );
                                  }
                                  
                                  if (trimmedLine.startsWith('Completed Services:')) {
                                    const value = trimmedLine.replace('Completed Services:', '').trim();
                                    return (
                                      <div key={lineIndex} className="flex items-start gap-3">
                                        <span className="text-gray-500 font-medium min-w-[120px]">Completed Services:</span>
                                        <span className="text-primary font-bold">{value}</span>
                                      </div>
                                    );
                                  }
                                  
                                  if (trimmedLine.startsWith('Extra Charges Total:')) {
                                    const value = trimmedLine.replace('Extra Charges Total:', '').trim();
                                    return (
                                      <div key={lineIndex} className="flex items-start gap-3 mt-4 pt-4 border-t border-gray-300">
                                        <span className="text-gray-500 font-medium min-w-[120px]">Extra Charges Total:</span>
                                        <span className="text-orange-600 font-bold text-lg">{value}</span>
                                      </div>
                                    );
                                  }
                                  
                                  if (trimmedLine.startsWith('Extra Charges Details:')) {
                                    return (
                                      <div key={lineIndex} className="mt-3">
                                        <span className="text-gray-600 font-semibold">{trimmedLine}</span>
                                      </div>
                                    );
                                  }
                                  
                                  if (trimmedLine.startsWith('- ')) {
                                    const chargeText = trimmedLine.substring(2);
                                    return (
                                      <div key={lineIndex} className="ml-6 flex items-start gap-2">
                                        <span className="text-orange-500 mt-1">•</span>
                                        <span className="text-gray-800">{chargeText}</span>
                                      </div>
                                    );
                                  }
                                  
                                  if (trimmedLine.startsWith('Notes:')) {
                                    const notesText = trimmedLine.replace('Notes:', '').trim();
                                    return (
                                      <div key={lineIndex} className="mt-4 pt-4 border-t border-gray-300">
                                        <div className="flex items-start gap-3 mb-2">
                                          <span className="text-gray-600 font-semibold min-w-[120px]">Notes:</span>
                                        </div>
                                        {notesText && (
          ...
          <Card className="p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              All project services are pre-selected. Uncheck any services that were not completed.
            </p>
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          ...
                          <div className="text-right ml-4">
                            <p className="font-semibold text-primary">${(task.estimatedCost || 0).toFixed(2)}</p>
                            <p className="text-xs text-gray-500">{(task.estimatedHours || 0)}h</p>
                          </div>
                        </div>
            ...
            <div className="space-y-3">
              {extraCharges.map((charge, index) => (
                <div key={index} className="flex gap-3 items-start p-3 border rounded-lg bg-gray-50">
                  <div className="flex-1">
                    <Label htmlFor={`charge-desc-${index}`} className="text-sm text-gray-600">
                      Description
                    </Label>
                    <Input
                  ...
                  <div className="w-32">
                    <Label htmlFor={`charge-amount-${index}`} className="text-sm text-gray-600">
                      Amount ($)
                    </Label>
                    <Input
          ...
          <Card className="p-6 sticky top-6">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="pb-3 border-b">
                ...
                <p className="text-xl font-bold text-primary">${servicesCost.toFixed(2)}</p>
              </div>

              {extraChargesTotal > 0 && (
                <div className="pb-3 border-b">
                  <p className="text-sm text-gray-600">Extra Charges</p>
                  <p className="text-xl font-bold text-orange-600">${extraChargesTotal.toFixed(2)}</p>
                </div>
              )}
                ...
                <div className="space-y-2 max-h-[150px] overflow-y-auto mb-4">
                  {selectedTasksDetails.map((task) => (
                    <div key={task.taskId} className="flex items-start justify-between text-sm">
                      ...
                      <span className="text-primary font-medium ml-2">${(task.estimatedCost || 0).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {extraCharges.filter(charge => charge.description.trim() && charge.amount > 0).length > 0 && (
              <div className="mb-6">
                <p className="text-sm font-semibold text-gray-700 mb-2">Extra Charges:</p>
                <div className="space-y-2 max-h-[150px] overflow-y-auto">
                  {extraCharges
                    .filter(charge => charge.description.trim() && charge.amount > 0)
                    .map((charge, index) => (
                      <div key={index} className="flex items-start justify-between text-sm">
                        <span className="text-gray-700 flex-1">{charge.description}</span>
                        <span className="text-orange-600 font-medium ml-2">${charge.amount.toFixed(2)}</span>
                      </div>
                    ))}
                </div>
              </div>
