...
export default function CustomerReportsPage() {
  ...
  const loadTasksForAppointment = async (appointmentId: number) => {
    try {
      ...
      console.log(
        `Found ${appointmentTasks.length} tasks for appointment ${appointmentId}`,
        appointmentTasks
      );
      ...
      setCompletedAppointments((prev) =>
        prev.map((apt) =>
          apt.id === appointmentId ? { ...apt, tasks: appointmentTasks } : apt
        )
      );
    } catch (err) {
  ...
  const loadTasksForProject = async (projectId: number) => {
    try {
      ...
      setProjectReports((prev) =>
        prev.map((proj) =>
          proj.id === projectId ? { ...proj, tasks: projectTasks } : proj
        )
      );
    } catch (err) {
        ...
        {projectReports.length > 0 ? (
          <div className="grid grid-cols-1 gap-6">
            {projectReports.map((project) => {
              ...
              const reportData = project.description
                ? parseProjectReport(project.description)
                : null;
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                          ...
                          {reportData && (
                            <div className="flex items-center gap-2 text-gray-700">
                              ...
                              <span className="text-sm">
                                Completed: {reportData.completedServices}{" "}
                                services
                              </span>
                            </div>
                          )}
          ...
          <Card className="p-8 text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Project reports will appear here once your completed projects have
              reports submitted.
            </p>
          </Card>
        )}
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex flex-col gap-2">
                        {!isExpanded && (
                          <Button
                            onClick={() =>
                              loadTasksForAppointment(appointment.id)
                            }
                            disabled={isLoadingTasks}
                            variant="outline"
                            size="sm"
